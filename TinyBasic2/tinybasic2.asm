...
RTSTV   .EQU 7                          ;*** RST 7 @ $0038 ***
        ...
        JC   QHOW                       ;IS INDEX TOO BIG (>0x7FFF)?
...
TV1:    CPI  21H                        ;>='a'?
        JC   TV2                        ;NO
        SUI  20H                        ;MAKE UPPER CASE
TV2:    CPI  1BH                        ;NOT @, IS IT A TO Z?
...
PRINT:  MVI  C,8                        ;C = # OF SPACES
        MVI  A,10                       ;DEFAULT BASE FOR PRTNUM
        STA  PNBASE
...
PR6:    MVI  A,10                       ;END OF LIST REACHED
        STA  PNBASE                     ;RESET DEFAULT BASE
...
SIZE:   LHLD TXTUNF                     ;*** RETURN SIZE IN HL ***
...
PEEK:   CALL PARN                       ;*** PEEK(EXPR) ***
        MOV  A,M                        ;GET CONTENT OF (HL)
        MVI  H,0                        ;RETURN RESULT IN HL
        MOV  L,A
        RET

DEEK:   CALL PARN                       ;*** PEEK(EXPR) ***
        PUSH D
        MOV  E,M                        ;GET LOW CONTENT OF (HL)
        INX  H
        MOV  D,M                        ;GET HIGH CONTENT OF (HL)
...
GL1:    CALL CHKIO                      ;CHECK KEYBOARD
        ...
        CPI  BS                         ;BS, DELETE LAST CHARACTER?
        JZ   GL3                        ;YES
        CPI  DEL                        ;DEL, DELETE LAST CHARACTER?
        JZ   GL3                        ;YES
        ...
        ORA  A                          ;IGNORE NULL
        JZ   GL1
        CPI  CAN                        ;^X, DELETE THE WHOLE LINE?
        JZ   GL4                        ;YES
...
GL3:    MOV  A,E                        ;DELETE LAST CHARACTER
        ...
        MVI  A,BS                       ;AND ECHO A BACKSPACE
        RST  ROUTC
        MVI  A,' '                      ;AND ECHO A BLANK
        RST  ROUTC
        MVI  A,BS                       ;AND ECHO A BACKSPACE
        RST  ROUTC
        JMP  GL1                        ;GO GET NEXT INPUT
GL4:    CALL CRLF                       ;REDO ENTIRE LINE
        MVI  A,'^'                      ;CR, LF AND UP-ARROW
        JMP  GETLN
;
FNDLN:  MOV  A,H                        ;*** FNDLN ***
        ORA  A                          ;CHECK SIGN OF HL
...
PRTNUM: PUSH D                          ;*** PRINT NUMBER IN HL ***
        LDA  PNBASE                     ;GET NUMBER BASE
        MVI  D,0                        ;16 BIT FOR DIVIDE
        MOV  E,A
        MOV  B,D                        ;NO PREFIX YET
        CPI  16                         ;HEX NUMBER?
        JNZ  PN0                        ;NO, CHECK SIGN
        MVI  B,'$'                      ;PRINT LEADING '$'
        DCR  C                          ;'$' TAKES SPACE
        JMP  PN1                        ;HEX IS UNSIGNED
...
PN6:    LDA  PNBASE                     ;GET NUMBER BASE
        CMP  E                          ;IT IS FLAG FOR NO MORE
        MOV  A,E                        ;CHECK DIGIT IN E
        POP  D
        RZ                              ;IF SO, RETURN
        CPI  10                         ;0-9? < A hex?
        JC   PN7                        ;Skip Add 7
        ADI  'A'-'0'-10                 ;Bring it up to ASCII A-F
PN7:    ADI  '0'                        ;ELSE CONVERT TO ASCII
        RST  ROUTC                      ;AND PRINT THE DIGIT
        JMP  PN6                        ;GO BACK FOR MORE

PRTLN:  MVI  A,10                       ;SET 10 AS DEFAULT BASE
        STA  PNBASE                     ;FOR PRTNUM
        LDAX D
...
EX1:    LDAX D                          ;IF FOUND '.' IN STRING
        ...
        JZ   EX3
        CPI  'a'                        ;< 'a' ?
        JC   EXN                        ;NO ALPHA CHAR
        CPI  'z'+1                      ;> 'z'
        JNC  EXN                        ;NO ALPHA CHAR
        ANI  5FH                        ;MASK LOWER CASE TO UPPER CASE
EXN:
        INX  H                          ;HL->TABLE
